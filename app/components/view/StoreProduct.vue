<template>
  ...
  <div class="main-comp-item" :class="enable_widget ? '' : 'disabled-row'">
    ...
    <div class="remove-btn" v-if="isDelete" @click="deleteProduct">
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'url'">
          <img
            :src="record.url"
            :alt="record.name"
            style="'opacity: 0.2; width: 30px; height: 30px'"
          />
        </template>
...
export default {
  ...
  mounted() {},
  methods: {
    onSearchProduct: _.debounce(function (searchText) {
      this.dataSearch = [];
      this.isSearch = true;
    ...
    addProduct(item) {
      ...
      if (this.dataTable.length < 5) {
        const isExist = this.dataTable.some(
    ...
    deleteProduct() {
      ...
      this.dataTable = this.dataTable.filter(
        (item) => !this.rows.includes(item.key)
      );
      this.rows = [];
      this.isDelete = false;
